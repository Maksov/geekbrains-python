...
def even(s):
    return ' '.join(element for element in s.split(' ')[1::2])
...
# Решите задачу в одну строчку используя срезы.
def reverse(s):
    return ' '.join(element for element in s.split(' ')[::-1])
...
def shift(s):
    str_list = s.split(' ')
    return ' '.join(element for element in ([str_list.pop()] + str_list))
...
def inside(s):
    str_list = s.split(' ')
    first_last_items = [str_list.pop(0)] + [str_list.pop(-1)]
    return ' '.join(str_list[:int(len(str_list) / 2)] +
                    first_last_items + str_list[int(len(str_list) / 2):])
