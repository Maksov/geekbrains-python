...
# Примечание: попробуйте записать решение в одну строку
# при помощи генератора списка и встроенной функции sum
def multiples():
...
# полученное произведением двух двухзначных чисел – 9009 = 91 * 99.
# Найдите самый большой палиндром,
# полученный произведением двух трёхзначных чисел.
def palindrome():
    nums = (x * y for x, y in product(range(999, 0, -1), range(999, 0, -1)))
    return max(x for x in nums if str(x) == str(x)[::-1])
